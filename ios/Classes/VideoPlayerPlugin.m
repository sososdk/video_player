...
@property(nonatomic, readonly) bool isSendInitialized;
@property(nonatomic, readonly) bool isSeeking;
@property(nonatomic, readonly) CMTime seekPosition;
...
- (instancetype)initWithPlayerItem:(AVPlayerItem*)item frameUpdater:(FLTFrameUpdater*)frameUpdater {
    ...
    if (@available(iOS 10.0, *)) {
        //        _player.currentItem.preferredForwardBufferDuration = 30;
...
- (void)observeValueForKeyPath:(NSString*)path
    ...
    if (context == timeRangeContext) {
        ...
        if ([[_player currentItem] isPlaybackLikelyToKeepUp]) {
            [self updatePlayingState];
            if (_eventSink != nil) {
                _eventSink(@{@"event" : @"keepUpStart"});
            }
        } else {
            if (_eventSink != nil) {
                _eventSink(@{@"event" : @"keepUpEnd"});
            }
        }
...
- (void)sendInitialized {
    if (_eventSink && _isInitialized && !_isSendInitialized) {
        _isSendInitialized = true;
...
- (int64_t)position {
    if (_isSeeking) {
        return FLTCMTimeToMillis(_seekPosition);
    } else {
...
- (void)seekTo:(int)location {
    _isSeeking = true;
    _seekPosition = CMTimeMake(location, 1000);
    [_player seekToTime:_seekPosition
